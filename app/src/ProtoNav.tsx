    ...
    <div style={{ position: 'fixed', right: 16, bottom: 16, zIndex: 100, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
      {s.protoOpen && (
        <div style={{ background: '#241f1a', border: '1px solid #3a332b', borderRadius: 14, padding: 6, boxShadow: '0 12px 40px rgba(0,0,0,.55)', display: 'flex', flexDirection: 'column', minWidth: 190 }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: '#9a9084', padding: '8px 12px 4px' }}>Jump to screen</div>
          {SCREENS.map(([k, label]) => (
          ...
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: '#9a9084', padding: '4px 12px' }}>Meeting preview (no connection)</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px', fontSize: 13, color: '#c9beb0' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {(['host', 'guest'] as const).map(r => (
                <button key={r} onClick={() => app.patch(st => ({ devRole: r, isHost: st.devMode ? r === 'host' : st.isHost }))} style={{ background: s.devRole === r ? '#2e2822' : 'none', border: '1px solid #3a332b', color: s.devRole === r ? '#f0a97f' : '#968a7b', borderRadius: 8, padding: '4px 10px', fontSize: 11.5, fontWeight: 600, cursor: 'pointer' }}>{r}</button>
              ))}
            </div>
          </div>
      ...
      <button className="hv-fg" onClick={() => app.patch(st => ({ protoOpen: !st.protoOpen }))} title="Prototype map" style={{ width: 40, height: 40, borderRadius: '50%', background: '#241f1a', border: '1px solid #3a332b', color: '#968a7b', cursor: 'pointer', boxShadow: '0 8px 24px rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
